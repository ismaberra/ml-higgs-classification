...
class DecisionTreeClassifier:
    ...
    def _information_gain(
        ...
        left_impurity = (
            self._gini_impurity(y_left)
            if self.criterion == "gini"
            else self._entropy(y_left)
        )
        right_impurity = (
            self._gini_impurity(y_right)
            if self.criterion == "gini"
            else self._entropy(y_right)
        )
    ...
    def _find_best_split(
        ...
        for feature_idx in range(n_features):
            if feature_idx in self.categorical_features:
                ...
                n_thresholds = min(
                    5, len(np.unique(feature_values))
                )  # Limit to 5 thresholds max
                if n_thresholds <= 1:
                    continue

                # Use quantile-based sampling for better distribution
                thresholds = np.percentile(
                    feature_values, np.linspace(25, 75, n_thresholds)
                )
    ...
    def _build_tree(self, X: np.ndarray, y: np.ndarray, depth: int = 0) -> DecisionNode:
        ...
        if (
            ...
            or len(y) < 2 * self.min_samples_leaf  # Early stopping for small nodes
            or (len(y) > 10000 and depth > 3)
        ):  # Early stopping for very large datasets
            # Create leaf node
            prediction = Counter(y).most_common(1)[0][0]
            return DecisionNode(prediction=prediction)

        # Find best split
        best_feature, best_threshold, best_gain, best_is_categorical = (
    ...
    def _compute_importances_recursive(
        self, node: DecisionNode, X: np.ndarray, y: np.ndarray
    ):
...
class RandomForestClassifier:
    ...
    def _select_features(self, n_features: int) -> List[int]:
        ...
        return self.rng_.choice(
            n_features, size=min(n_selected, n_features), replace=False
        )
        ...
        for i in range(self.n_estimators):
            ...
            tree = DecisionTreeClassifier(
                ...
                categorical_features=[
                    j for j in self.categorical_features if j in selected_features
                ],
        ...
        for tree, selected_features in self.estimators_:
            for i, feature_idx in enumerate(selected_features):
                if i < len(tree.feature_importances_):
                    self.feature_importances_[feature_idx] += tree.feature_importances_[
                        i
                    ]
